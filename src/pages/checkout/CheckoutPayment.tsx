...
import { HinhThucThanhToanModel } from "../../models/LookupModels"; // model của bạn
import dinhDangSo from "../../layouts/Utils/dinhDangSo";
...
const CheckoutPayment: React.FC = () => {
    ...
    const checkoutState = location.state as {
        diaChiNhanHang: string;
        diaChiMuaHang: string;
        maHinhThucGiaoHang: number; // GIỮ nguyên: đây là ID giao hàng đã chọn ở bước 2
    };

    useEffect(() => {
        // Validate state từ bước 2
        if (
            !checkoutState ||
            checkoutState.maHinhThucGiaoHang === undefined ||
            checkoutState.maHinhThucGiaoHang === null
        ) {
            console.error(
                "CheckoutPayment: Thiếu thông tin địa chỉ hoặc mã giao hàng từ bước trước."
            );
            setError("Lỗi dữ liệu thanh toán. Vui lòng quay lại bước nhập địa chỉ.");
            setLoading(false);
            return;
        ...
        (async () => {
            try {
                setLoading(true);
                setError(null);
                const data = await layHinhThucThanhToan();
                // data: [{ maHinhThucThanhToan, tenHinhThucThanhToan, moTa, chiPhiThanhToan }]
                setPaymentMethods(Array.isArray(data) ? data : []);
                if (Array.isArray(data) && data.length > 0) {
                    const defaultId = Number(data[0].maHinhThucThanhToan); // ✅ dùng đúng key thanh toán
                    setSelectedMethodId(defaultId);
                    console.log("Default payment method ID set:", defaultId);
                } else {
                    setError("Không tìm thấy hình thức thanh toán nào.");
                }
            } catch (e) {
                console.error("Lỗi tải hình thức thanh toán:", e);
                setError("Không thể tải danh sách hình thức thanh toán. Vui lòng thử lại.");
            } finally {
                setLoading(false);
            }
    ...
    const handleMethodChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const newId = Number(event.target.value); // ✅ ép number
        setSelectedMethodId(newId);
        console.log("Payment Method Selected:", newId);
    };

    const handlePlaceOrder = async () => {
        console.log("ID thanh toán đã chọn:", selectedMethodId);
        ...
        // ✅ Gửi đúng 4 field mà BE yêu cầu
        const orderData: CheckoutRequestData = {
            diaChiMuaHang: checkoutState.diaChiMuaHang,
            diaChiNhanHang: checkoutState.diaChiNhanHang,
            maHinhThucGiaoHang: Number(checkoutState.maHinhThucGiaoHang),
            maHinhThucThanhToan: Number(selectedMethodId), // ✅ đúng key THÀNH TOÁN
        };

        console.log(">>> PAYLOAD GỬI LÊN BE:", JSON.stringify(orderData, null, 2));

        try {
            const createdOrder = await datHangAPI(orderData);
            console.log("Đơn hàng tạo thành công:", createdOrder);
            // xóa giỏ / thông báo tùy bạn
            window.dispatchEvent(new CustomEvent("cartUpdated"));
            navigate(`/dat-hang-thanh-cong/${createdOrder.maDonHang}`, { replace: true });
        } catch (err: any) {
    ...
    if (error) {
        return (
            <div className="container mt-5">
                <div className="alert alert-danger">{error}</div>
                <Link to="/thanh-toan/dia-chi" className="btn btn-outline-primary">
                    Quay lại Bước 1
                </Link>
            </div>
        );
    ...
    return (
        <div className="container mt-4 mb-5" style={{ maxWidth: "700px" }}>
            ...
            <div className="card mb-3">
                <div className="card-body">
                    {paymentMethods.map((method) => {
                        // ✅ DÙNG ĐÚNG KEY CỦA HÌNH THỨC THANH TOÁN
                        const methodId = Number(method.maHinhThucThanhToan);
                        const methodTen = method.tenHinhThucThanhToan;
                        const methodMoTa = method.moTa;
                        const methodPhi = Number(method.chiPhiThanhToan);
                    ...
                    <span>
                      <strong>{methodTen}</strong>
                        {methodMoTa && <small className="d-block text-muted">{methodMoTa}</small>}
                    </span>
                                        {methodPhi > 0 && (
                                            <span className="fw-bold">+{dinhDangSo(methodPhi)} đ</span>
                                        )}
            ...
            <div className="d-flex justify-content-between mt-3">
                ...
                >
                    {isPlacingOrder ? (
                        ...
                        <>
                            Đặt Hàng <i className="fas fa-check ms-1" />
                        </>
                    )}
                </button>
            </div>
